...
train_evaluate_tree <- function(train_data, test_data, department_name) {
  ...
  pdp_targeted <- partial(pruned_tree, pred.var = "targeted_productivity", train = train_data)
  pdp_incentive <- partial(pruned_tree, pred.var = "incentive", train = train_data)
  
  # Plot and save PDP for targeted_productivity
  pdp_targeted_plot <- autoplot(pdp_targeted) +
    theme_minimal() +
    labs(title = paste(department_name, " - Partial Dependence on Targeted Productivity"),
         x = "Targeted Productivity", y = "Predicted Productivity")
  ggsave(paste0(department_name, "_pdp_targeted_productivity_(incentive_tp).png"), plot = pdp_targeted_plot)
  
  # Plot and save PDP for incentive
  pdp_incentive_plot <- autoplot(pdp_incentive) +
    theme_minimal() +
    labs(title = paste(department_name, " - Partial Dependence on Incentive"),
         x = "Incentive", y = "Predicted Productivity")
  ggsave(paste0(department_name, "_pdp_incentive_(incentive_tp).png"), plot = pdp_incentive_plot)
