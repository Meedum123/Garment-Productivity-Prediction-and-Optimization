...
train_evaluate_tree <- function(train_data, test_data, department_name) {
  ...
  png(paste0(department_name, "_pruned_tree.png"))
  rpart.plot(pruned_tree, main = paste(department_name, " - Pruned Decision Tree"))
  dev.off()
  ...
  if (!is.null(pruned_tree$variable.importance)) {  # Check if importance exists
    ...
    # Save the plot
    ggsave(paste0(department_name, "_feature_importance.png"), plot = feature_plot)
  } else {
  ...
  for (feature in predictors) {
    pdp_data <- partial(pruned_tree, pred.var = feature, train = train_data)
    pdp_plot <- autoplot(pdp_data) +
      labs(title = paste(department_name, " - Partial Dependence Plot for", feature),
           x = feature, y = "Partial Dependence")
    ggsave(paste0(department_name, "_pdp_", feature, ".png"), plot = pdp_plot)
  }
