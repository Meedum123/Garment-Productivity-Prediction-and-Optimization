...
#------------------ Fit Lasso Regression for Sewing -----------------------------
x_s = x_Train[sewing_Train, ]
y_s = y_Train[sewing_Train]

fit.lasso.s <- glmnet(x_s, y_s, alpha = 1)
cv.lasso.s <- cv.glmnet(x_s, y_s, alpha = 1)
bestlam.s <- cv.lasso.s$lambda.min 

# Predictions and accuracy check
pred_s_train = predict(fit.lasso.s, s = bestlam.s, newx = x_s)
mse_s_train = mean((y_s - pred_s_train)^2)
r2_s_train = calc_r2(y_s, pred_s_train)

pred_s_test = predict(fit.lasso.s, s = bestlam.s, newx = x_Test[sewing_Test, ])
mse_s_test = mean((y_Test[sewing_Test] - pred_s_test)^2)
r2_s_test = calc_r2(y_Test[sewing_Test], pred_s_test)

#------------------ Fit Lasso Regression for Finishing -----------------------------
x_f = x_Train[finishing_Train, ]
y_f = y_Train[finishing_Train]

fit.lasso.f <- glmnet(x_f, y_f, alpha = 1)
cv.lasso.f <- cv.glmnet(x_f, y_f, alpha = 1)
bestlam.f <- cv.lasso.f$lambda.min 

# Predictions and accuracy check
pred_f_train = predict(fit.lasso.f, s = bestlam.f, newx = x_f)
mse_f_train = mean((y_f - pred_f_train)^2)
r2_f_train = calc_r2(y_f, pred_f_train)

pred_f_test = predict(fit.lasso.f, s = bestlam.f, newx = x_Test[finishing_Test, ])
