...
fit.ridge.s <- glmnet(x_s[train_s, ], y_s[train_s], alpha = 1)
plot(fit.ridge.s, xvar = "lambda", label = TRUE, lw = 2)

# Cross-validation to find best lambda
cv.ridge.s <- cv.glmnet(x_s[train_s, ], y_s[train_s], alpha = 1)
plot(cv.ridge.s)    
...
fit.ridge.f <- glmnet(x_f[train_f, ], y_f[train_f], alpha = 1)
plot(fit.ridge.f, xvar = "lambda", label = TRUE, lw = 2)

# Cross-validation to find best lambda
cv.ridge.f <- cv.glmnet(x_f[train_f, ], y_f[train_f], alpha = 1)
plot(cv.ridge.f)    
